...
<tbody class="header">
    <?php
        ...
        if(isset($_POST['cari'])) {
			...
			$query = mysql_query("
                ...
                WHERE nama_customer LIKE '%$kunci%'
                OR almt_customer LIKE '%$kunci%'
                OR id_customer LIKE '%$kunci%'
            ");
        ...
        $jatuh_tempo = $db->queryUniqueValue('select max(tgl_jatuh_tempo) from piutang where nama_customer="'.$data["nama_customer"].'"');
			  $piutang = $db->queryUniqueValue('select sum(debit) - sum(kredit) from piutang where nama_customer="'.$data["nama_customer"].'"');
    ...
    <tr>
        ...
        <td class="jatuhtempo" name="<?php echo $i?>"><?php if ($jatuh_tempo!='0000-00-00 00:00:00'&&$jatuh_tempo!='') { echo date('d-m-Y',strtotime($jatuh_tempo)); } ?></td>
		<td class="harga" id="harga<?php echo $i?>"><?php echo number_format($piutang,0,'','.'); ?></td>
