...
<tbody class="header">
    <?php
        ...
        while($data = mysql_fetch_array($query)) {
            $no_po = $data['no_po'];
            $query1 = mysql_query("SELECT statusdibayar from pembelian where no_po ='$no_po'");
            $statuspembelian = mysql_fetch_array($query1);
            $no_penjualan = $data['no_penjualan'];
            $query2 = mysql_query("SELECT statusdibayar from penjualan where no_penjualan ='$no_penjualan'");
            $statuspenjualan = mysql_fetch_array($query2);
            $statusbayarpembelian =$statuspembelian[0];
            $statusbayarpenjualan =$statuspenjualan[0];
    ...
    <tr>
		...
		<td>
		<?php if ($data['no_penjualan']!=''&&$data['no_po']==0 && $statusbayarpembelian != 'LUNAS') {?>		
		<a href="memopesanan.php?id=<?php echo $data['no_penjualan']?>" id="<?php echo $data['no_penjualan']?>" class="ubah" data-toggle="modal">
                <i class="fa fa-edit"><small>Memo Pesanan</small></i>
        </a>
		<a href="tambahpembelian.php?id=<?php echo $data['no_penjualan']?>" id="<?php echo $data['no_penjualan']?>" class="ubah" data-toggle="modal">
                <i class="fa fa-edit"><small>Tambah Pembelian</small></i>
        </a>
		<?php } ?>

		<?php if ($data['no_penjualan']!=''&&$data['no_po']!=0 && $statusbayarpembelian != 'LUNAS') {?>		
		<a href="memopembelian.php?id=<?php echo $data['no_penjualan']?>" id="<?php echo $data['no_penjualan']?>" class="ubah" data-toggle="modal">
                <i class="fa fa-edit"><small>Memo Pembelian</small></i>
        </a>
            <?php } 
           if ($data['hargajualtotal']==0 && $data['no_penjualan']!=''&&$data['no_po']!=0 && $statusbayarpenjualan != 'LUNAS'){?>
		<a href="tambahpenjualan.php?id=<?php echo $data['no_penjualan']?>" id="<?php echo $data['no_penjualan']?>" class="ubah" data-toggle="modal">
                <i class="fa fa-edit"><small>Tambah Penjualan</small></i>
        </a>
		<?php } ?>
		<?php if ($data['hrgjualdgnpajak']!=0 && $statusbayarpenjualan != 'LUNAS') {?>
		<a href="memopenjualan.php?id=<?php echo $data['no_penjualan']?>" id="<?php echo $data['no_penjualan']?>" class="ubah" data-toggle="modal">
                <i class="fa fa-edit"><small>Memo Penjualan</small></i>
        </a>
		<?php } ?>		
		</td>
    </tr>
